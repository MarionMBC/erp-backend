...
export const deleteProductCategory = async (req, res) => {
    try {
        const id = req.params.id;
        const [productCategory] = await pool.query('DELETE FROM productCategories WHERE id=?', [id]);
...
export const createProductCategory = async (req, res) => {
    try {
        const {name, status} = req.body;
        const [productCategory] = await pool.query('INSERT INTO productCategories (name, status) VALUES (?, ?)', [name, status]);
        return productCategory.affectedRows > 0 ?
            res.status(200).json({
...
export const getProductCategoryStatus = async (req, res) => {
    try {
        const [productCategory] = await pool.query('SELECT * FROM productCategories WHERE status = 1');
        return productCategory.length > 0 ?
            res.status(200).json(productCategory) : res.status(404).json({
                msg: 'No se ha encontrado la categoría de producto.'
            })
...
export const getProductCategoryStatusFalse = async (req, res) => {
    try {
        const [productCategory] = await pool.query('SELECT * FROM productCategories WHERE status = 0');
        return productCategory.length > 0 ?
            res.status(200).json(productCategory) : res.status(404).json({
                msg: 'No se ha encontrado la categoría de producto.'
            })
